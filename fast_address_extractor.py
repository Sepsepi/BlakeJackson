...
async def human_pause(action_type="normal"):
    """OPTIMIZED: Ultra-fast pauses for maximum speed"""
    if action_type == "typing":
        # Minimal typing pause
        delay = random.uniform(0.01, 0.05)
    elif action_type == "reading":
        # Very fast result reading
        delay = random.uniform(0.2, 0.4)
    elif action_type == "between_searches":
        # ULTRA-SHORT pause between searches for maximum speed
        delay = random.uniform(0.1, 0.3)
    elif action_type == "page_load":
        # Quick page load wait
        delay = random.uniform(0.2, 0.5)
    else:
        # Minimal default pause
...
async def smart_typing(page, selector, text):
    """OPTIMIZED: Ultra-fast typing with minimal delays"""
    await page.fill(selector, '')
    await human_pause("typing")

    # Very fast typing with minimal character delays
    for i, char in enumerate(text):
        await page.type(selector, char, delay=random.randint(10, 30))  # Much faster: 10-30ms vs 80-200ms

        # Rarely pause (like occasional thinking)
        if random.random() < 0.02:  # Reduced from 0.1 to 0.02
            await human_pause("typing")

        # Skip typo simulation for speed (was causing extra delays)
        # Removed typo correction logic for maximum speed

async def add_human_behavior(page):
    """OPTIMIZED: Minimal human behavior for speed"""
    # Skip random scrolling for maximum speed
    # Original code caused unnecessary delays
    pass
...
async def score_name_match(search_name, found_name, property_name=None):
    ...
    Enhanced to handle middle initial variations (e.g., "John J" matches "John Joseph")
    """
    ...
    def matches_first_name(search_first, found_text):
        """Check if first name matches, including middle initial variations"""
        if not found_text:
            return False

        # Direct match
        if search_first in found_text:
            return True

        # Check for middle initial patterns
        # e.g., "John" should match "John J" or "John Joseph"
        words = found_text.split()
        for word in words:
            if word == search_first:
                return True
            # Check if search name is a middle initial version
            # e.g., search "John J" should match found "John Joseph"
            if len(search_first) == 1 and word.startswith(search_first):
                return True

        return False

    def matches_last_name(search_last, found_text):
        """Check if last name matches"""
        if not found_text:
            return False

        # Direct match or word boundary match
        if search_last in found_text:
            return True
        ...
        for word in words:
            ...
            if search_last in word and len(search_last) > 3:
                return True

        return False

    # Check owner section (found_name) with enhanced matching
    has_first_owner = matches_first_name(search_first, found_lower)
    has_last_owner = matches_last_name(search_last, found_lower)
    ...
    if property_name:
        ...
        has_first_property = matches_first_name(search_first, property_lower)
        has_last_property = matches_last_name(search_last, property_lower)
    ...
    # Parse found name more carefully for owner section with enhanced middle initial matching
    if owner_match:
        if ',' in found_lower:
            ...
            # Enhanced first name matching with middle initial support
            if found_first_part:
                first_words = found_first_part.split()
                exact_first_match = False
                middle_initial_match = False

                # Check for exact first name match
                if search_first in first_words:
                    exact_first_match = True
                    score += 30  # Exact first name match

                # Check for middle initial patterns
                # Case 1: Search "John" should match found "John J" or "John Joseph"
                if not exact_first_match and len(first_words) >= 2:
                    if first_words[0] == search_first:
                        # Found name starts with our search first name
                        second_word = first_words[1]
                        if len(second_word) == 1:  # Middle initial like "J"
                            middle_initial_match = True
                            score += 35  # Bonus for middle initial match
                            print(f"    [MATCH] Middle initial detected: '{search_first}' matches '{first_words[0]} {second_word}'")
                        elif len(second_word) > 1:  # Full middle name like "Joseph"
                            middle_initial_match = True
                            score += 25  # Full middle name match
                            print(f"    [MATCH] Middle name detected: '{search_first}' matches '{first_words[0]} {second_word}'")

                # Case 2: Search "John J" should match found "John Joseph"
                if not exact_first_match and not middle_initial_match and len(search_parts) >= 3:
                    # Check if our search has a middle initial (format: "First Middle Last")
                    if len(search_parts) >= 3:
                        search_middle = search_parts[1]
                        if search_middle and len(search_middle) == 1 and len(first_words) >= 2:
                            if (first_words[0] == search_first and
                                len(first_words[1]) > 1 and
                                first_words[1].startswith(search_middle)):
                                score += 40  # High bonus for middle initial expansion match
                                print(f"    [MATCH] Middle initial expansion: '{search_first} {search_middle}' matches '{first_words[0]} {first_words[1]}'")

                # Fallback partial match
                if not exact_first_match and not middle_initial_match and search_first in found_first_part:
                    score += 15  # Partial first name match
        else:
            # Handle "FIRST LAST" format with enhanced matching
            found_words = found_lower.split()

            # Look for exact word matches and middle initial patterns
            exact_first_match = False
            middle_initial_match = False
            ...
            # Check for middle initial patterns in "FIRST MIDDLE LAST" format
            if not exact_first_match and len(found_words) >= 3:
                if found_words[0] == search_first:
                    # Check if second word could be middle initial/name
                    second_word = found_words[1]
                    if len(second_word) == 1:  # Middle initial
                        middle_initial_match = True
                        score += 30
                        print(f"    [MATCH] Middle initial in full format: '{search_first}' matches '{found_words[0]} {second_word}'")
                    elif len(second_word) > 1:  # Full middle name
                        middle_initial_match = True
                        score += 20
                        print(f"    [MATCH] Middle name in full format: '{search_first}' matches '{found_words[0]} {second_word}'")

            if search_last in found_words:
                score += 25

    # Enhanced scoring for property section matches with middle initial support
    if property_match and property_name:
        ...
        if ',' in property_lower:
            ...
            if prop_first_part:
                ...
                # Check for middle initial patterns in property section
                elif len(prop_first_words) >= 2 and prop_first_words[0] == search_first:
                    second_word = prop_first_words[1]
                    if len(second_word) == 1:  # Middle initial
                        score += 30
                        print(f"    [MATCH] Property middle initial: '{search_first}' matches '{prop_first_words[0]} {second_word}'")
                    elif len(second_word) > 1:  # Full middle name
                        score += 20
                        print(f"    [MATCH] Property middle name: '{search_first}' matches '{prop_first_words[0]} {second_word}'")
        else:
            ...
            # Check middle initial patterns in property full format
            if len(prop_words) >= 3 and prop_words[0] == search_first:
                second_word = prop_words[1]
                if len(second_word) == 1:  # Middle initial
                    score += 25
                    print(f"    [MATCH] Property full format middle initial: '{search_first}' matches '{prop_words[0]} {second_word}'")
                elif len(second_word) > 1:  # Full middle name
                    score += 15
                    print(f"    [MATCH] Property full format middle name: '{search_first}' matches '{prop_words[0]} {second_word}'")
...
async def process_addresses_fast(csv_path, max_names: Optional[int] = None, headless=True):
    ...
    def clean_name_for_search(full_name):
        """
        Clean name for optimal search - remove suffixes, middle names, etc.
        Focus on first and last name only for better matching
        """
        if not full_name:
            return None

        # Common suffixes to remove
        suffixes = ['JR', 'SR', 'II', 'III', 'IV', 'V', 'ESQ', 'PHD', 'MD', 'DDS', 'RN']
        ...
        if ',' in full_name:
            ...
            if first_part:
                # Remove suffixes and get just the first name
                words = first_part.upper().split()
                # Remove known suffixes
                clean_words = [word for word in words if word not in suffixes]
                if clean_words:
                    first_name = clean_words[0]  # Take just the first name
            ...
            if len(words) >= 2:
                ...
                # Take first and last words (skip middle names)
                if len(words) >= 2:
                    return f"{words[0]} {words[-1]}"
                elif len(words) == 1:
                    return words[0]  # Only one name left
    ...
    # Use Cleaned versions (first + last only) instead of FullCleaned (with middle names)
    ...
    for _, row in df.iterrows():
        ...
        # Priority: IndirectName_Cleaned for person names (first + last only)
        if row.get('IndirectName_Type') == 'Person' and row.get('IndirectName_Cleaned'):
            primary_name = row['IndirectName_Cleaned']  # This is first + last only

            # Use the FullCleaned version as fallback if it's different and has more detail
            if row.get('IndirectName_FullCleaned') and row['IndirectName_FullCleaned'] != primary_name:
        ...
        # Fallback: DirectName_Cleaned for person names (first + last only)
        elif row.get('DirectName_Type') == 'Person' and row.get('DirectName_Cleaned'):
            primary_name = row['DirectName_Cleaned']  # This is first + last only

            # Use the FullCleaned version as fallback if it's different and has more detail
            if row.get('DirectName_FullCleaned') and row['DirectName_FullCleaned'] != primary_name:
                fallback_name = row['DirectName_FullCleaned']

            # Check if IndirectName_Cleaned exists as potential additional fallback
            elif row.get('IndirectName_Cleaned'):
        ...
        if primary_name:
            ...
            if fallback_name and pd.notna(fallback_name):
                ...
                print(f"  [DEBUG] {primary_name} (cleaned) -> fallback: {fallback_name}")

    unique_names = sorted(list(set(person_names)))
    print(f"[✓] Found {len(unique_names)} unique person names (using cleaned first+last only)")
    ...
    if max_names and len(unique_names) > max_names:
        ...
        print(f"[✓] Processing ALL {len(unique_names)} names (first+last only)")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # OPTIMIZED: Skip pauses between searches for maximum speed
            # Original pause removed to achieve ultra-fast processing
    ...
    # Fill in addresses based on cleaned names (first + last only)
    for index, row in df.iterrows():
        address_found = None

        # Try to match using IndirectName_Cleaned (priority) - first + last only
        if row.get('IndirectName_Cleaned'):
            ...
            # Also check FullCleaned name in case it was used as fallback
            elif row.get('IndirectName_FullCleaned'):
        ...
        # Try DirectName_Cleaned if we haven't found an address yet - first + last only
        if not address_found and row.get('DirectName_Cleaned'):
            ...
            # Also check FullCleaned name in case it was used as fallback
            elif row.get('DirectName_FullCleaned'):
...
async def main():
    ...
    parser.add_argument('--max-names', type=int, default=None, help='Maximum number of names to process (default: unlimited)')
